...
class AIIntegration:
    ...
    def _generate_github_actions_workflow(self, file_path, description, requirements, dependencies):
        """Generate a GitHub Actions workflow YAML file"""
        logger.info("Generating GitHub Actions workflow YAML")
        
        with open(file_path, 'w') as f:
            f.write(f'''name: Whisper MP4 Transcription Test
...
''')
    ...
    def process_task(self):
        ...
        # Generate file based on the task requirements
        ...
        # Get template type and other details
        template_type = self.task.get('template_type')
        file_type = self.task.get('file_type')
        requirements = self.task.get('requirements', [])
        dependencies = self.task.get('dependencies', [])
        description = self.task.get('description', 'AI-generated content')
        
        # Create parent directories if needed
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        
        # Generate appropriate content based on template_type
        if template_type == 'github_actions_workflow' or (
            target_file.startswith('.github/workflows/') and 
            (file_type == 'yaml' or file_type == 'yml' or 
             target_file.endswith('.yml') or target_file.endswith('.yaml'))
        ):
            self._generate_github_actions_workflow(file_path, description, requirements, dependencies)
        else:
