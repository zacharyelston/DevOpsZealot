#!/bin/bash
# Docker wrapper for Whisper CLI validator
# This script runs the Whisper CLI validator in a Docker container
...
if [ "$3" = "--auto-fix" ] || [ "$3" = "--autofix" ]; then
  AUTO_FIX=true
  log_info "Auto-fix mode enabled - will attempt to fix Whisper CLI integration issues"
fi
...
log_info "Building Docker image for Whisper CLI validator"
...
COPY validators/whisper_cli_validator.py /app/
EOF

# Build the Docker image
docker build -t whisper-cli-validator-image -f "$TMP_DOCKERFILE" "$SCRIPT_DIR"
rm "$TMP_DOCKERFILE"
...
docker run --rm \
  ...
  whisper-cli-validator-image python /app/whisper_cli_validator.py $VALIDATOR_ARGS

EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
  log_info "SUCCESS: All Whisper CLI integration validations passed successfully"
else
  if [ "$AUTO_FIX" = true ]; then
    log_info "Auto-fix attempted for Whisper CLI integration issues"
  else
    log_warning "Whisper CLI integration validation failed. Run with --auto-fix to fix issues"
  fi
fi
