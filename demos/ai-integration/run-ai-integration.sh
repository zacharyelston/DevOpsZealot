#!/bin/bash
# AI Integration Demo Runner for DevOpsZealot
# This script demonstrates the core functionality of sending files to an AI API,
# receiving improvements, and committing the changes to a repository.
...
# Display banner
echo -e "${BLUE}=============================================${NC}"
echo -e "${BLUE}DevOpsZealot AI Integration Demo${NC}"
echo -e "${BLUE}=============================================${NC}"
...
(cd "${DEMO_REPO_DIR}" && git checkout -b "${BRANCH_NAME}")
...
if [ $EXIT_CODE -eq 0 ]; then
  ...
  echo -e "  cd ${DEMO_REPO_DIR} && git push origin ${BRANCH_NAME}"
else
  echo -e "\n${RED}=============================================${NC}"
  echo -e "${RED}AI Integration Demo failed with exit code ${EXIT_CODE}!${NC}"
  echo -e "${RED}=============================================${NC}"
  echo -e "\n${YELLOW}To reset the repository:${NC}"
  echo -e "  cd ${DEMO_REPO_DIR} && git checkout master && git branch -D ${BRANCH_NAME}"
fi
